...
resource "digitalocean_droplet" "k8s_master" {
  ...
  user_data          = "${data.template_file.etcd-k8s-master.rendered}"
   # Generate the Certificate Authority
    ...
    provisioner "local-exec" {
        command = "$PWD/cfssl/generate_server.sh k8s_master '${digitalocean_droplet.k8s_master.ipv4_address},${digitalocean_droplet.k8s_master.ipv4_address_private},10.3.0.1,kubernetes.default,kubernetes'"
    }
    ...
    provisioner "local-exec" { command = "$PWD/cfssl/generate_client.sh k8s_master" }
